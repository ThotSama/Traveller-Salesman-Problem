...
iterations = 300
pygame.font.init()
#--RECUPERATION DES DONNEES DES POINTS -----------------------------#
graph = data
#-------------------------------------------------------------------#
...
class Manager(object):
    ...
    def ShowText(self, selectedIndex, started = True):
        ...
        if started == False:
            self.screen.blit(textSurface3, (width//4, height-200))

    def DrawShortestPath(self):
        if len(self.OptimalRoutes) > 0:
            for j in range(self.n_points):
                _i = (j+1)%self.n_points
                pygame.draw.line(self.screen, self.Highlight,
                                (self.OptimalRoutes[j].x, self.OptimalRoutes[j].y),
                                (self.OptimalRoutes[_i].x, self.OptimalRoutes[_i].y),
                                self.LineThickness)
                self.OptimalRoutes[j].Draw(self, self.showIndex, True, j)
